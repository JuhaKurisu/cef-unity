...
### 2026-07-23 ステップ1 (即効枠 + 旧 Phase 0) — 修正済み 16 件

- **SRV-N20** ✅ record_paint_latency 先頭に LOG_ENABLED 早期 return
- **SRV-N21** ✅ create_browser / resize で `clamp(1, MAX_W/MAX_H)` (ipc 定数参照)
- **SRV-3** ✅ macos.rs: IN_TICK (thread_local) 再入ガード + PANICKED (AtomicBool) 分離。panic 経路で `&mut` を再作成しない。tick 順序・1ms interval は不変。※ctx.info 正規経路化 (修正案1) は未採用 — 再入ガードで UB 芽は解消済み
- **SRV-9** ✅ server.rs の `.lock().unwrap()` 全 19 箇所を `unwrap_or_else(PoisonError::into_inner)` に + catch_unwind payload の downcast ログ。※panic::set_hook は未導入
- **IPC-9** ✅ write_frame は usize 拡幅乗算 + 次元 assert、read 系 2 箇所は次元チェック先行 (u32 wrap 経路の根絶)
- **CLI-3** ✅ accept を別スレッド + 15s deadline + child.try_wait() 監視。新エラーコード: **-6 = server 早期死亡、-7 = タイムアウト**。Child は ServerConnection が保持
- **CLI-14/IPC-2 (部分)** ✅ shutdown の 500ms 後に try_wait でゾンビ回収 (通常ケース)。ACK 化は未実施
- **CLI-N23** ✅ shutdown が CONNECTION ガードを先に解放してから sleep
- **CLI-11 (短期)** ✅ legacy Metal 経路を @autoreleasepool で包んだ。ABI 削除 (中期) は未実施
- **CLI-N20** ✅ build.rs を `rerun-if-changed=src` (ディレクトリ) に + コメントの事実誤認修正。**touch 検証済み: .m 変更で再コンパイルされる = 「cargo clean 必須」問題根治**
- **AUD-1** ✅ Unity 側 ThrowIfDisposed を `ObjectDisposedException(nameof(Browser))` に (csharp 側は既に ThrowIf 使用で正しかった — drift の実例)
- **AUD-2** ✅ pull_trampoline に catch_unwind (panic 時は無音 fill + PULL_PANIC_COUNT)
- **AUD-3** ✅ NativeVoice.ctx を `Box::into_raw` の raw 所有に (Drop で stop → from_raw)。既存テスト 12 件パス
- **SCR-6** ✅ cef_scroll_monitor_poll 先頭に null/max≤0 ガード (lib.rs 側)
- **SCR-7** ✅ start_impl 冒頭で `g_count = 0` (残留リング掃除)
