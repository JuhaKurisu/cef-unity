...
**[実装時改訂 2026-07-20]** 固定 5ms オフセット + 2 点履歴は実機で棄却された。macOS のスクロールイベントは表示レート (60Hz ≈ 16.7ms 間隔) で届くため、イベント無しフレームで sample が外挿上限に当たり hold→ジャンプのビートジッターが出る (実測: 粗さ median 0.147、排出が -16,-14,-9,-18 とジグザグ)。**確定実装: サンプルオフセットをイベント間隔の EMA×1.25 (5〜25ms にクランプ) に適応させ、イベント履歴を 4 点保持**することでサンプルが常に補間帯域に入る (修正後 median 0.088、ユーザー体感「カクツキ消失」で採用)。遅延は約 1 イベント間隔 (60Hz なら ~21ms) の固定値で、テール/浮遊なし・momentum 終端即停止は維持。

- イベントから軸ごとの累積位置 P(t) を構築 (直近 4 イベントの (t, P) を保持)
- 毎フレーム `sampleTime = source.Now − 適応オフセット` の P を求める:
  - 2 イベント間なら**線形補間**
...
## 実装後の発展 (2026-07-21〜22, ユーザー A/B 駆動)

- **予測モードを既定採用 (2026-07-22)**: 補間モード (適応オフセット ~1 イベント間隔) の固定遅延はユーザーに体感されたため、`Predictive` モードを追加し既定化。サンプル = now−5ms、外挿上限 = イベント間隔×1.25、**no-backtrack** (進行方向と逆の補正はサンプル位置を保持し、終端フラッシュは永続方向 `_lastDir` でガード)。dev トグル `cef_scroll_interp` で補間モードにオプトアウト可。クラス既定は false (テスト互換)、統合側で true
- **傾き発散の修正**: macOS はジェスチャ→慣性遷移で ~0.2ms 差の連続イベント (GestureEnded dy=0 → MomentumBegan) を送り、2点傾きが発散して数千 px のスパイクを排出していた (実測 5099px)。**MergeEpsilon=2ms** (近接イベントは同一点にマージ) + **外挿速度を履歴窓全体 (4点端点間) から算出** で解消 (同一録画のリプレイ実測: max|dy| 5099→182px)
- **録画+リプレイ検証基盤** (`cef_scroll_record` → `$TMPDIR/cef_scroll_events.csv`、`CefUnity.Editor.ScrollReplay.Run` batchmode 実行): 実トラックパッド操作の生イベント列を本物の ScrollResampler にオフラインで通し、補間/予測両モードの排出を比較検証できる。リプレイ忠実度は実測で完全 (>2px 乖離ゼロ)。**以後の入力系不具合はユーザーの手を借りずに再現・修正・検証できる**
